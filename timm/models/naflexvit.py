...
class NaFlexVit(nn.Module):
    ...
    def forward(
        ...
        patch_coord: Optional[torch.Tensor] = None,
        patch_valid: Optional[torch.Tensor] = None,
        return_cls: bool = False,  # 新增参数
    ) -> torch.Tensor:
        """Forward pass with optional NaFlex support.

        Args:
            x: Input tensor or dict from NaFlex collator
            patch_coord: Optional patch coordinates
            patch_valid: Optional patch validity indicators
            return_features: If True, return backbone features instead of classification logits
        """
        if isinstance(x, Dict):
            patch_coord = x['patch_coord']
            patch_valid = x['patch_valid']
        ...
        if return_cls:
            # feats shape could be [B, C] or [B, N, C]
            if feats.ndim == 3:
                feats = feats.mean(dim=1)  # 平均所有 token -> [B, C]
            return feats
